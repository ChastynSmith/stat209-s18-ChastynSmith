...
```{r}
diamondmodel <- lm_basic(price ~ 1 + carat, data = diamonds) 
reg_table(diamondmodel)
```

How would you interpret the slope coefficient in this model?

**Answer**: The slope coefficient shows that it would be around $7000 more for each additional carat. 
...
**Answer**: Yes, because that slope estimate does not fall within the range for the slope.

Would you be surprised if we collected another set of diamonds, in exactly the
same way as before, and found a slope estimate equal to 7500?

**Answer**: Also yes, because this value also is not within the confidence interval.
...
**Answer**: It is important because it lets us know if something for sure increases or decreases. The slope in the linear regression lets us know that the cost of each carat increases the price for a fact somewhere between $7729-$7784.   
...
```{r}
depthmodel<- lm_basic(price ~ 1 + depth, data = diamonds) 
reg_table(depthmodel, level = 0.95)
```

What does the model predict will be the price of a diamond with a depth of
60?

**Answer**: It predicts the price will be $3,984. 67.

Describe in words the estimate of the slope in terms of the dataset.

**Answer**: The slope shows that as the depth increases the price of the diamond will decrease by $29.65.
...
**Answer**: the confidence interval conatains numbers that are only negative.

Interpret this in words (your words, not just the formal definition).

**Answer**: This means that the depth of a diamond for sure decreases the price. It can decrease the price anywhere from $53 dollars to only $6 dollars.
...
**Answer**: It will cost $14,465.5.

Are both of the slopes in the model statistically significant? What are
their signs?

**Answer**: Yes both of the slopes are statistically significant, both range values for the depth are negative and both range values for the carat are positive.
...
**Answer**: The largest residual is 12,683.655.
...
**Answer**: The slopes show the difference in price for the increase by 1 of each variable. 
...
**Answer**: I expect the heavier diamond to cost $7,765.10 more. 

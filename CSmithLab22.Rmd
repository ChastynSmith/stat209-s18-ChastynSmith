...
**Answer**: The 3rd percentile is 369.0 seconds and the 97th percentile is 4107.46 seconds.
...
```{r}
speedmodel <- lm_basic(perfectionism ~ 1 + speed, data = pf)
reg_table(speedmodel, level = 0.95)
```
...
**Answer**: Both slopes are significant. The perfectionism score increases for normal speed takers and decreases very slighlty for slow speed takers.
...
speedmodel <- lm_basic(perfectionism ~ 1 + speed_normal, data = pf)
reg_table(speedmodel, level = 0.95)

...
**Answer**: This number shows the average percent or number of crimes done that ended up in an arrest. This could be taken as a precentage. 
...
**Answer**: This shows the average amount/number of crimes that were classified as theft.
...
**Answer**: This shows all of the crimes that were considered theft from the entire dataset.
...
**Answer**: This number shows the amount of crimes classified as theft out of all of the households in the dataset.
...
**Answer**: This average is bigger than the arrest flag amount over the entire dataset. We can compare these datasets because they are both average values and is being presented as a value in relationship to the size of the dataset.
...
**Answer**: This number is smaller than the theft flag amount over the entire datset. Yes, we can compare these measurements because they are both average values and is being presented as a value in relationship to the size of the dataset.
...
**Answer**: This number is bigger than the measuremnet over the entire dataset. I think we can saafely compare these measurements because both are kind of like the average. Also both measurements were taken in relationship to their own dataset. In a way mean taking the average sample size was considered.
...
**Answer**: R is using the area number to match up the datasets.
...
```{r}
crimes_ca$theft_rate <- (crimes_ca$theft_mean / crimes_ca$num_households)*1000
```
...
**Answer**: The general pattern of this graph is that the theft rate stays somewhat consistent no matter the median income. There area few outliers that show a high theft rate among the households between the medians $0-$40,000. 
...
**Answer**: Fuller park has a high unemployment rate. This means that some people may not have an income therefore are not bale to purchase things even basic necessities. Therefore to get things people may resort to theft.
